...
def play_bell(bears, message, channel, down_times, delay_time):
...
class DataListener(OpenRTM_aist.ConnectorDataListenerT):
        ...
        def __init__(self, name, channel, bears, down_times, delay_time):
                self._name = name
        ...
        def __call__(self, info, cdrdata):
                ...
                play_bell(self.bears, data, self._channel, self.down_times, self._delay_time)
...
class MIDIHandBell(OpenRTM_aist.DataFlowComponentBase):
	...
	def onInitialize(self):
		...
		self.bindParameter("channel", self._channel, "-1")
		self.bindParameter("delay_time", self._delay_time, "0")
	...
	def onStartup(self, ec_id):
                print("channel: %d port: %s baudrate: %d down_pos: %d up_pos: %d delay_time: %d" % (self._channel[0],
                                                                                                    self._port[0],
                                                                                                    self._baudrate[0],
                ...
                self._midi_inIn.addConnectorDataListener(OpenRTM_aist.ConnectorDataListenerType.ON_RECEIVED,
                                                         DataListener("ON_RECEIVED", self._channel[0], self.bears, self.down_times, self._delay_time[0]))
